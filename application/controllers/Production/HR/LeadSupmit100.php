...
{ 
    ...
    public function index_post() {
        ...
        $sql = "UPDATE TSR_DB1.dbo.SaleTeam_Work100_Detail SET LeadCheckTime = ?, LeadApproveStatus = ?, UpdateDate = ?, UpdateBy = ?, LeadCause = ?
                WHERE DetailID = ?";
        ...
        if ($this->db->affected_rows() > 0) {
            $sql = "UPDATE TSR_DB1.dbo.SaleTeam_Work100 SET LeadCheckNum = (
                        SELECT COUNT(TeamID) FROM TSR_DB1.dbo.SaleTeam_Work100_Detail WHERE TeamID = " . $teamid . "
                    ), LeadApproveNum = (
                        SELECT COUNT(TeamID) FROM TSR_DB1.dbo.SaleTeam_Work100_Detail WHERE TeamID = " . $teamid . " AND LeadApproveStatus = 1
                    ), LeadNotApproveNum = (
                        SELECT COUNT(TeamID) FROM TSR_DB1.dbo.SaleTeam_Work100_Detail WHERE TeamID = " . $teamid . " AND LeadApproveStatus = 0
