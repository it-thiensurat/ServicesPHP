...
class AddSale extends REST_Controller {
    ...
    public function index_post() {
        ...
        if ($checksaleid != 'N') {
            if ($checkcitiid == 0) {
                ...
                $stmt = $this->db->insert('TSR_DB1.dbo.SaleTeam_Work_Detail', $data);
                    if ($stmt) {
                            $this->response(
                                array(
                                    'status'    => 'SUCCESS'
    ...
    public function checkSaleCitizenID($newid) {
        $sql = "SELECT TOP 1 SL.SaleStatus
                ...
                LEFT JOIN TSR_Application.dbo.NPT_Sale_Log AS SL ON SL.SaleID = SM.SaleID AND SL.LogID = (SELECT MAX(SS.LogID)
    ...
    public function checkNewCitizenID($newid) {
        $sql = "SELECT TOP 1 TeamID
                FROM TSR_DB1.dbo.SaleTeam_Work_Detail WITH(NOLOCK)
                WHERE CitizenID = ?
