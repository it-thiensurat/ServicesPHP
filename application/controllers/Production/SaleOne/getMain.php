...
class getMain extends REST_Controller {
    ...
    function index_get() { 
       ...
       $methodType = $this->input->get('methodType');

        if($methodType === "SaleData") {

            $this->getSaleData();

        } else if ($methodType === "SaleEmpData") {
    ...
    function saveSaleData() {
       ...
       /* Insert Master */
       $sql = "Insert into TSR_DB1.dbo.SaleOne_WorkLog (TeamCode,TeamNum,TeamWorkNum,TeamOutNum,ipaddress,computername) values (?,?,?,?,?,?)";
       ...
       foreach($SaleEmpList as $key){
            ...
            $sql = "Insert into TSR_DB1.dbo.SaleOne_WorkLog_Details (MasterIDX,SaleType,EmpID,EmpName,SaleCode,WorkStatus,ipaddress,computername) values (?,?,?,?,?,?,?,?)";
     ...
     foreach($SaleEmpOutList as $key){
        ...
        $sql = "Insert into TSR_DB1.dbo.SaleOne_WorkLog_Details (MasterIDX,SaleType,EmpName,WorkStatus,ipaddress,computername) values (?,?,?,?,?,?)";
