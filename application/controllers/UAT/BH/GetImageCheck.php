...
{ 
    public function __construct()
    {
        ...
        $this->baseUrl = "http://appuat.thiensurat.co.th/mca/images/NewcontractImage/";
    }
    ...
    public function index_post() {
        ...
        $sql        = "SELECT ac.RefNo, c.CONTNO, c.EFFDATE, p.ProductName, p.ProductModel, c.ProductSerialNumber, ac.comment, ac.createby, ac.Approve,
                        ...
                        FROM SQLUAT.TSR_Application.dbo.ApproveContno ac
                        LEFT JOIN SQLUAT.Bighead_Mobile.dbo.Contract c ON ac.RefNo = c.RefNo
                        LEFT JOIN SQLUAT.Bighead_Mobile.dbo.Product p ON c.ProductID = p.ProductID
                        LEFT JOIN SQLUAT.Bighead_Mobile.dbo.DebtorCustomer d ON c.CustomerID = d.CustomerID
    ...
    public function getImageDetail($refno) {
        $sql = "SELECT ai.id, ai.RefNo, ai.createby, ai.ImageTypeCode, CONCAT('http://appuat.thiensurat.co.th/mca/images/NewcontractImage/', ai.ImageName) AS ImageName, 
                ai.ProblemId, ap.ProblemNameV3 
                FROM SQLUAT.TSR_Application.dbo.ApproveImageType AS ai
                LEFT JOIN SQLUAT.TSR_ONLINE_MARKETING.dbo.v_AllProblem AS ap ON ai.ProblemId = ap.ProblemIDV3
