...
{ 
    public function __construct()
    {
        ...
        $this->baseUrl = "http://uat.thiensurat.co.th/mca2/images/NewcontractImage/";
    }
    ...
    public function index_post() {
        ...
        $sql        = "SELECT ac.RefNo, c.CONTNO, c.EFFDATE, p.ProductName, p.ProductModel, c.ProductSerialNumber, ac.comment, ac.createby, ac.Approve,
                        ...
                        LEFT JOIN Bighead_Mobile.dbo.Product p ON c.ProductID = p.ProductID
                        LEFT JOIN Bighead_Mobile.dbo.DebtorCustomer d ON c.CustomerID = d.CustomerID
                        WHERE ac.createby = ? AND ac.status = 1 and ac.Approve IN (1, 2, 3) AND CONVERT(VARCHAR(10), ac.createdate,126) = CONVERT(VARCHAR(10),GETDATE(),126)";
        $stmt       = $this->db->query($sql, array($empid));
        if ($stmt->num_rows() > 0) {
            $data = [];
            $result = $stmt->result_array();
    ...
    public function getImageDetail($refno) {
        // $sql = "SELECT ai.id, ai.RefNo, ai.createby, ai.ImageTypeCode, CONCAT('http://uat.thiensurat.co.th/mca2/images/NewcontractImage/', ai.ImageName) AS ImageName, 
