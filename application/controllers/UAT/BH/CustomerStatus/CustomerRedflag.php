...
class CustomerRedflag extends REST_Controller
{ 
    ...
    $sql = "SELECT Refno ,CONTNO ,IDCard ,PrefixName ,CustomerName ,PayLastStatus ,CustomerStatus ,AccountStatus
            ,PayType ,AllPeriods ,PayLastPeriod ,TotalPrice ,ProductName ,ProductModel ,SaleCode ,EffDate
            ,AgingCumulative ,AgingContinuous ,AgingCumulativeDetail ,StDate ,df ,TelHome ,TelMobile
            FROM 
            (
                ...
                INNER JOIN (
                    SELECT CustomerName ,IDCard ,max(EffDate) as effdate
                    FROM TSR_Application.dbo.DebtorAnalyze_Master AS M
                    LEFT JOIN TSR_Application.dbo.DebtorAnalyze_Address AS DA ON M.Refno = DA.Refno AND DA.AddressTypeCode = 'AddressIDCard'
                    WHERE M.IDCard = '" . $searchValue . "' OR M.CustomerName LIKE '%" . $searchValue . "%' OR DA.TelMobile = '" . $searchValue . "' OR DA.TelHome = '" . $searchValue . "'
                    GROUP BY CustomerName ,IDCard
                ) AS CS ON CS.CustomerName = M.CustomerName AND CS.IDCard = M.IDCard AND CS.effdate = M.EffDate
