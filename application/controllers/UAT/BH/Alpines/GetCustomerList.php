...
{ 
    ...
    public function index_get() {
        ...
        if (!isset($empId)) {
            $sql    = "SELECT *, (SELECT TOP 1 aq.APQ_ID FROM SQLUAT.TSR_DB1.dbo.ALPINE_CUSTOMER ac 
                    ...
                    FROM SQLUAT.TSR_DB1.dbo.ALPINE_CUSTOMER ac1
                    LEFT JOIN SQLUAT.TSR_DB1.dbo.ALPINE_CUSTOMER_ADDRESS AS ad ON ac1.APCUS_ID = ad.APCUS_ID AND ad.APADDR_TYPE = 'AddressIDCard'";
            $stmt   = $this->db->query($sql);
        } else {
            $sql    = "SELECT *, ('') AS APQ_ID FROM SQLUAT.TSR_DB1.dbo.ALPINE_CUSTOMER AS ac
                        LEFT JOIN SQLUAT.TSR_DB1.dbo.ALPINE_CUSTOMER_ADDRESS AS ad ON ac.APCUS_ID = ad.APCUS_ID AND ad.APADDR_TYPE = 'AddressIDCard'
    ...
    public function index_post() {
        ...
        if (!isset($empId)) {
            // $sql    = "SELECT *, 
            //             (SELECT TOP 1 aq.APQ_ID FROM SQLUAT.TSR_DB1.dbo.ALPINE_CUSTOMER ac 
            //                 LEFT JOIN SQLUAT.TSR_DB1.dbo.ALPINE_QUOTATION aq ON ac.APCUS_ID = aq.APCUS_ID 
    ...
    public function getProduct($quotationId) {
        $sql = "SELECT * FROM SQLUAT.TSR_DB1.dbo.ALPINE_QUOTATION_DETAILS WHERE APQD_STATUS = 1";
        $stmt = $this->db->query($sql);
        if ($stmt->num_rows() > 0) {
