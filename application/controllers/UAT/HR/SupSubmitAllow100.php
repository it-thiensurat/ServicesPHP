...
{ 
    ...
    public function index_post() {
        ...
        // if ($this->db->affected_rows() > 0) {
            $sql = "UPDATE SQLUAT.TSR_DB1.dbo.SaleTeam_Work100 SET SupCheckNum = (
                        SELECT COUNT(TeamID) FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail WHERE TeamID = " . $teamid . "
                    ), SupApproveNum = (
                        SELECT COUNT(TeamID) FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail WHERE TeamID = " . $teamid . " AND SupApproveStatus = 1
                    ), SupNotApproveNum = (
                        SELECT COUNT(TeamID) FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail WHERE TeamID = " . $teamid . " AND SupApproveStatus = 0
    ...
    public function updateToCostBranch($citizen, $fnno, $fnyear, $amount, $approvestatus, $empid, $causestatus) {
        $sql = "exec SQLUAT.TSR_Application.dbo.CostBranch_Manage_Allowance_100 @citizenid = ?,
