...
{ 
    ...
    public function index_post() {
        ...
        $sql = "SELECT DISTINCT SL.saleemp, SL.salecode, SL.FName + ' ' + SL.LName as Fullname, SM.CitizenID
                ...
                AND SL.PosID < 3 AND SL.TeamNo IS NOT NULL AND SL.SaleStatus != 'R' AND SL.DepID = ? AND SL.FnYear = ? AND SL.FnNo = ? AND ISNULL(SL.SaleEmpType1, 0) != 8";
    ...
    public function createMasterData($teamno, $empid, $fnyear, $fnno, $depid, $teamlist, $paydate) {
        ...
        if (!$this->checkCreateMaster($teamno, $depid, $fnno, $fnyear)) {
            $id = "0";
            $stmt = $this->db->insert('SQLUAT.TSR_DB1.dbo.SaleTeam_Work100', $data);
            if ($stmt) {
                $stmt = $this->db->query("SELECT CONVERT(varchar, MAX(TeamID)) AS TeamID FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100");
                $id = $stmt->row()->TeamID;
                foreach($teamlist as $k => $v) {
                    ...
                    if ($result) {
                        ...
                        if ($result->amount > 0) {
                            $stmt = $this->db->insert('SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail', $data);
                        }
                    }
                }

                // if ($this->db->affected_rows() > 0) {
                    $sql = "SELECT * FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail AS swd WHERE swd.TeamID = ?";
            ...
            $team = [];
            $teamid = "";
            $sql = "SELECT TOP 1 * FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail
    ...
    public function checkCreateMaster($teamno, $depid, $fnno, $fnyear) {
        $sql = "SELECT TOP 1 * FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100 WHERE TeamCode = ? AND DepID = ? AND FnNo = ? AND FnYear = ? 
    ...
    public function getAmountPay($date, $citizen) {
        $sql = "exec SQLUAT.TSR_Application.dbo.CostBranch_Allowance_100 ?, ?";
    ...
    public function getApproveStatus($teamno, $depid, $fnno, $fnyear) {
        $sql = "SELECT LeadApproveStatus FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100 WHERE TeamCode = ? AND DepID = ? AND FnNo = ? AND FnYear = ? 
