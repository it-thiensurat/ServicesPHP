...
class LeadApproveBFAddSale extends REST_Controller
{
              ...
              if (!$this->checkCreateMaster($teamno, $depid, $fnno, $fnyear)) {
                  ...
                  if ($stmt) {
                      ...
                      foreach($team_list as $k => $v) {
                          ...
                          $stmt2 = $this->db->insert('SQLUAT.TSR_DB1.dbo.SaleTeam_Work_Detail', $detail);
                      }

                      if ($this->db->affected_rows() > 0) {
                          $sql    = "UPDATE SQLUAT.TSR_DB1.dbo.SaleTeam_Work
                                     SET LeadCheckNum = (
                                          SELECT COUNT(sw.DetailID) AS total FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work_Detail AS sw
                                          WHERE sw.TeamID = '" . $id . "'
                                     ), LeadCheckWorkNum = (
                                          SELECT COUNT(sw.DetailID) AS approve FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work_Detail AS sw
                                          WHERE sw.TeamID = '" . $id . "' AND sw.LeadApproveStatus = 1
                                     ), LeadCheckOutNum = (
                                          SELECT COUNT(sw.DetailID) AS unapprove FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work_Detail AS sw
                  ...
                  $sql = "UPDATE SQLUAT.TSR_DB1.dbo.SaleTeam_Work_Detail SET LeadCheckTime = ?, Latitude = ?, Longitude = ?, LeadApproveStatus = ?, PaymentAmount = ?, UpdateDate = ?, UpdateBy = ?
                    ...
                    if ($this->db->affected_rows() > 0) {
                        $sql    = "UPDATE SQLUAT.TSR_DB1.dbo.SaleTeam_Work
                                   ...
                                   WHERE sw.TeamID = '" . $teamid . "' AND sw.FnNo = " . $fnno . " AND sw.FnYear = " . $fnyear . " AND sw.DepID = " . $depid . "
                                        ...
                                        WHERE sw.TeamID = '" . $teamid . "' AND sw.FnNo = " . $fnno . " AND sw.FnYear = " . $fnyear . " AND sw.DepID = " . $depid . "
                                   ), LeadCheckTime = ?, UpdateDate = ?, UpdateBy = ?
                                   WHERE TeamID = ? AND FnYear = ? AND FnNo = ? AND DepID = ? AND CONVERT(VARCHAR(10),CreateDate,126) = CONVERT(VARCHAR(10),GETDATE(),126)";
