    ...
    public function index_post() {
        ...
        if ($paymentType == "100") {
            $sql = "UPDATE SQLUAT.TSR_DB1.dbo.SaleTeam_Work100 SET PaymentCheckNum = (
                        ...
                        SELECT COUNT(stwd.TeamID) AS total FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work100 AS sw
                        INNER JOIN SQLUAT.TSR_DB1.dbo.SaleTeam_Work100_Detail AS stwd ON sw.TeamID = stwd.TeamID AND stwd.PaymentStatus = 0 AND stwd.SupApproveStatus = 1
                        WHERE sw.TeamID = " . $teamId . " AND sw.FnNo = " . $fnno . " AND sw.FnYear = " . $fnyear . " AND sw.DepID = " . $depId . "
                    ), PaymentAmount = (
            ...
            $sql = "UPDATE SQLUAT.TSR_DB1.dbo.SaleTeam_Work SET PaymentCheckNum = (
                        SELECT COUNT(stwd.TeamID) AS total FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work AS sw
                        LEFT JOIN SQLUAT.TSR_DB1.dbo.SaleTeam_Work_Detail AS stwd ON sw.TeamID = stwd.TeamID AND stwd.SupApproveStatus = 1
                        WHERE sw.TeamID = " . $teamId . " AND sw.FnNo = " . $fnno . " AND sw.FnYear = " . $fnyear . " AND sw.DepID = " . $depId . "
                    ), PaymentCheckWorkNum = (
                        SELECT COUNT(stwd.TeamID) AS total FROM SQLUAT.TSR_DB1.dbo.SaleTeam_Work AS sw
